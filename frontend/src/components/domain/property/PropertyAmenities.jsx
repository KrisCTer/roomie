...
const amenityIconMap = {
  ...
  "Khóa cửa an toàn": <Lock className="w-5 h-5" />,
  // Bedroom
  "Giường King Bed": <Bed className="w-5 h-5" />,
  "Giường Queen": <Bed className="w-5 h-5" />,
  "Tủ quần áo": <Sofa className="w-5 h-5" />,
  "Điều hòa không khí": <Wind className="w-5 h-5" />,
  "Máy sưởi": <Thermometer className="w-5 h-5" />,
  // Kitchen
...
const getAmenityIcon = (amenityName) => {
  return amenityIconMap[amenityName] || <CheckCircle className="w-5 h-5" />;
};

const getIconColor = (category) => {
  const colorMap = {
    homeSafety: "text-blue-600",
    bedroom: "text-teal-600",
    kitchen: "text-orange-600",
    others: "text-green-600",
  };
                ...
                <div key={group.key}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-2">
                    {group.items.slice(0, 6).map((item, idx) => (
                      ...
                      >
                        <div className={getIconColor(group.key)}>
                          {getAmenityIcon(item)}
                        </div>

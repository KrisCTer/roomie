  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-white border border-gray-100 rounded-xl hover:shadow-md transition-all">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-gray-500 mb-1">Posting date: {listing.date}</p>
        <p className="text-lg font-bold text-blue-600">{listing.price}</p>
      </div>
      <span className={`${getStatusColor(listing.status)} text-white px-4 py-1.5 rounded-full text-sm font-medium flex-shrink-0`}>
        {listing.status}
      </span>

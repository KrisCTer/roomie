...
public class ContractTemplateService {
    ...
    private String buildContractHtml(Contract contract, boolean includeSignatures) {
        ...
        String supplementaryAndAmendmentSection = buildSupplementaryAndAmendmentSection(contract);
        ...
        return """
                ...
                .formatted(
                        ...
                        depositInWords,
                        paymentDay,
                        // Supplementary terms and amendments
                        supplementaryAndAmendmentSection,
                        // Signature section
                        signatureSection);
    }

    private String buildSupplementaryAndAmendmentSection(Contract contract) {
        List<String> terms = contract.getSupplementaryTerms();
        List<com.roomie.services.contract_service.entity.ContractAmendment> amendments = contract.getAmendments();

        boolean hasTerms = terms != null && !terms.isEmpty();
        boolean hasAmendments = amendments != null && !amendments.isEmpty();

        if (!hasTerms && !hasAmendments) {
            return "";
        }

        StringBuilder termsHtml = new StringBuilder();
        if (hasTerms) {
            int index = 1;
            for (String term : terms) {
                ...
                termsHtml.append("<p class='article-content'><strong>9.")
                        .append(index++)
                        .append("</strong> ")
                        .append(escapeHtml(term.trim()))
                        .append("</p>");
            }
        }

        StringBuilder amendmentHtml = new StringBuilder();
        if (hasAmendments) {
            int index = 1;
            for (com.roomie.services.contract_service.entity.ContractAmendment amendment : amendments) {
                if (amendment == null
                        || amendment.getContent() == null
                        || amendment.getContent().trim().isEmpty()
                        || !"APPROVED".equals(amendment.getApprovalStatus())) {
                    continue;
                }

                String title = amendment.getTitle() == null || amendment.getTitle().trim().isEmpty()
                        ? "Bổ sung hợp đồng"
                        : amendment.getTitle().trim();

                String typeLabel = "POST_SIGN_ADDENDUM".equals(amendment.getAmendmentType())
                        ? "Phụ lục sau ký"
                        : "Sửa đổi trước ký";

                amendmentHtml.append("<p class='article-content'><strong>10.")
                        .append(index++)
                        .append("</strong> ")
                        .append(escapeHtml(title))
                        .append(" - <span class='italic'>")
                        .append(escapeHtml(typeLabel))
                        .append("</span></p>")
                        .append("<p class='article-content indent'>")
                        .append(escapeHtml(amendment.getContent().trim()))
                        .append("</p>");
            }
        }
        ...
        if (!termsHtml.isEmpty()) {
            sections.append("<div class='article'><h2 class='article-title'>ĐIỀU 9<br/>ĐIỀU KHOẢN PHỤ</h2>")
                    .append(termsHtml)
                    .append("</div>");
        }
        if (!amendmentHtml.isEmpty()) {
            sections.append(
                    "<div class='article'><h2 class='article-title'>ĐIỀU 10<br/>SỬA ĐỔI, BỔ SUNG / PHỤ LỤC</h2>")
                    .append(amendmentHtml)
                    .append("</div>");
        }
    ...
    public String buildSimpleContractHtml(Contract contract) {
        ...
        return """
                ...
                .formatted(
                        ...
                        property != null && property.getRentalDeposit() != null
                                ? formatCurrency(property.getRentalDeposit())
                                : "0",
    ...
    private String convertYearsToWords(String years) {
        if (years == null || years.equals("___"))
            return "___________";
